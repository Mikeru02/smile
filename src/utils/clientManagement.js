...
export default class ClientManagement {
    static allowClient(ip) {
        // Stop portal redirect for HTTP, HTTPS, and DNS
        ...
        runSpawnSync("iptables", ["-t", "nat", "-I", "PREROUTING", "-s", ip, "-p", "tcp", "--dport", "53", "-j", "RETURN"]);
    ...
    static revokeClient(ip) {
        // Restore portal redirect for HTTP, HTTPS, and DNS
