...
export function validateSignupForm(usernameInput, passwordInput, consentInput, submitBtn) {
    const isUsernameValid = usernameInput.value.trim() !== '';
    const isPasswordValid = passwordInput.value.trim() !== '';
    const isConsentChecked = consentInput.checked;

    submitBtn.disabled = !(isUsernameValid && isPasswordValid && isConsentChecked);
}